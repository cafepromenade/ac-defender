...
- Telemetry Alibi may delay only safe corrections using real Home Assistant/weather/Ontario Energy telemetry signals. It must clear on direct comfort needs and must never create fake telemetry or simulated cover events.
...
- Ontario Energy Peak Power Saver may delay only safe cooling commands using real Ontario Energy Home Assistant usage sensors. It must step aside when room/upstairs comfort needs cooling or when a command would save energy by raising the setpoint.
...
- Use the Energy page in a browser after Ontario Energy frontend changes; verify tabs, search/filter controls, charts, and mobile layout.
...
```text
docker@<your-docker-host>
```

Project path:

```text
/home/docker/ac-defender
```

Deployment command:

```bash
cd /home/docker/ac-defender
