...
- Alectra Peak Power Saver may delay only safe cooling commands using real Alectra Home Assistant usage sensors. It must step aside when room/upstairs comfort needs cooling or when a command would save energy by raising the setpoint.
...
- Use the Energy page in a browser after Alectra frontend changes; verify tabs, search/filter controls, charts, and mobile layout.
