        ...
        bool Json)
    {
        public static CliUsageOptions Create(string[] args, IConfiguration configuration)
        {
            ...
            return new CliUsageOptions(
                ...
                TryGetValue(args, "--power") ?? configuration["HomeAssistant:UsagePowerEntityId"] ?? "sensor.alectra_current_power",
                TryGetValue(args, "--energy") ?? configuration["HomeAssistant:UsageEnergyEntityId"] ?? "sensor.alectra_energy_today",
                TryGetValue(args, "--cost") ?? configuration["HomeAssistant:UsageCostEntityId"] ?? "sensor.alectra_cost_today",
                TryGetValue(args, "--hourly-cost") ?? configuration["HomeAssistant:UsageHourlyCostEntityId"] ?? "sensor.alectra_hourly_cost",
                TryGetValue(args, "--bill") ?? configuration["HomeAssistant:UsageCurrentBillEntityId"] ?? "sensor.alectra_current_bill",
                TryGetValue(args, "--bill-due") ?? configuration["HomeAssistant:UsageCurrentBillDueEntityId"] ?? "sensor.alectra_current_bill_due",
                TryGetValue(args, "--bill-status") ?? configuration["HomeAssistant:UsageCurrentBillStatusEntityId"] ?? "sensor.alectra_current_bill_status",
