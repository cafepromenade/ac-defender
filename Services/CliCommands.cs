        ...
        bool Json)
    {
        public static CliUsageOptions Create(string[] args, IConfiguration configuration)
        {
            ...
            return new CliUsageOptions(
                ...
                TryGetValue(args, "--power") ?? configuration["HomeAssistant:UsagePowerEntityId"] ?? "sensor.ontario_energy_current_power",
                TryGetValue(args, "--energy") ?? configuration["HomeAssistant:UsageEnergyEntityId"] ?? "sensor.ontario_energy_energy_today",
                TryGetValue(args, "--cost") ?? configuration["HomeAssistant:UsageCostEntityId"] ?? "sensor.ontario_energy_cost_today",
                TryGetValue(args, "--hourly-cost") ?? configuration["HomeAssistant:UsageHourlyCostEntityId"] ?? "sensor.ontario_energy_hourly_cost",
                TryGetValue(args, "--bill") ?? configuration["HomeAssistant:UsageCurrentBillEntityId"] ?? "sensor.ontario_energy_current_bill",
                TryGetValue(args, "--bill-due") ?? configuration["HomeAssistant:UsageCurrentBillDueEntityId"] ?? "sensor.ontario_energy_current_bill_due",
                TryGetValue(args, "--bill-status") ?? configuration["HomeAssistant:UsageCurrentBillStatusEntityId"] ?? "sensor.ontario_energy_current_bill_status",
